...
"use strict";
const Game = (0, _gameClassJsDefault.default).default || (0, _gameClassJsDefault.default);
const game = new Game();
...
function renderMessages() {
    ...
    if (st === "idle") startMsg.classList.remove("hidden");
    if (st === "win") winMsg.classList.remove("hidden");
    if (st === "lose") loseMsg.classList.remove("hidden");
}
function renderScore() {
    const st = game.getStatus();
    const val = game.getScore();
    if (st === "idle") setScoreText("");
    else if (st === "playing" && val === 0) setScoreText("");
    else setScoreText(String(val));
}
function renderAll() {
    renderField();
    renderMessages();
